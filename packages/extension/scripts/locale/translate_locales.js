...
async function translateLocales() {
	const updatedLocaleEn = jsonfile.readFileSync(path.join(__dirname, 'locale_en.json'));
	const newKeys = Object.keys(updatedLocaleEn);
	const targetDirRoot = path.join(__dirname, '../../src/app/_locales');
	for (let i = 0; i < locales.length; i++) {
		const locale = locales[i];
		const oldJsonFilePath = path.join(targetDirRoot, `/${locale}/messages.json`);
		...
		for (let j = 0; j < newKeys.length; j++) {
			...
			if (!oldJsonFile[newKey]) {
				...
				if (newKey !== 'appName') {
					...
					oldJsonFile[newKey] = {
						message: updatedLocaleEn[newKey].message,
						description: updatedLocaleEn[newKey].description,
					};
				}
